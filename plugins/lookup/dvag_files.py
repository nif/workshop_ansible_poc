...
class LookupModule(LookupBase):
    ...
    def run(self, terms, variables=None, **kwargs):
      ...
      try:
          ...
          dvag_files_password = None

          foreman_host_parameters = ans['data']['results']
          for param in foreman_host_parameters:
              if param['name'] == 'dvag_files_password':
                  dvag_files_password = param['value']

          if dvag_files_password is None:
            raise Exception(f"ERROR!! could not find dvag_files_password parameter")

      except Exception as err:
        raise AnsibleError(f"ERROR!! could not find dvag_files_password parameter")

      for term in terms:
          display.debug("lookup term: %s" % term)

          if term == 'dvag_files_password':
            ret.append(dvag_files_password)

          else:
            url = f'https://{foreman_server}/dvag_files/{term}'
            user = 'dvag_files'

            resp = None

            try:
                auth = HTTPBasicAuth(user, dvag_files_password)
